...
def parse_time_column(inp):
    ...
    # 2021-09-13
    if len(inp) ==15:
        dt = datetime.datetime.fromtimestamp(float(inp)/1000)
        return CTime(dt.year, dt.month, dt.day, 0, 0)
    if len(inp) == 10:
        year = int(inp[:4])
        month = int(inp[5:7])
        day = int(inp[8:10])
...
class CSV_API(CCommonStockApi):
    def __init__(self, code, k_type=KL_TYPE.K_DAY, begin_date=None, end_date=None, autype=None,colums=['time_key','open','close','high','low','volume','amount']):
    ...
    def get_kl_data(self):
        ...
        for line_number, line in enumerate(open(file_path, 'r')):
            if self.headers_exist and line_number == 0:
                continue
            data = line.strip("\n").split(",")
            dt :CTime = parse_time_column(data[self.time_column_idx])
            data[self.time_column_idx] = dt.to_str()
